...
export function MoneyAnalyzer({ initialExpenses, baseCurrency = "INR" }: { initialExpenses: ExpenseRecord[], baseCurrency?: string }) {
  ...
  return (
    <div className="mx-auto max-w-7xl p-5 sm:p-7 lg:p-9">
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Metric label="Tracked spending" value={formatCurrency(analytics.total, baseCurrency)} note={`${expenses.length} transactions`} />
        <Metric label="Biggest category" value={analytics.biggestCategory?.name ?? "None yet"} note={formatCurrency(analytics.biggestCategory?.value ?? 0, baseCurrency)} />
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <Signal text={analytics.highestDay ? `${analytics.highestDay.day} was your highest spending day at ${formatCurrency(analytics.highestDay.value, baseCurrency)}.` : "Add a few expenses to find your highest spending day."} />
            <Signal text={unusual ? `${unusual.merchant} is your largest single expense at ${formatCurrency(unusual.amount * (unusual.exchangeRate || 1), baseCurrency)}.` : "Your unusual spending signal will appear here."} />
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody>{filtered.map((expense) => <tr key={expense.id} className="border-b last:border-0"><td className="py-3 text-muted-foreground">{format(new Date(expense.date), "d MMM yy")}</td><td className="py-3 font-medium">{expense.merchant}</td><td className="py-3"><span className="rounded-full bg-muted px-2 py-1 text-xs">{expense.category}</span></td><td className="max-w-[220px] truncate py-3 text-muted-foreground">{expense.notes || "—"}</td><td className="py-3 text-right font-semibold">{expense.currency && expense.currency !== baseCurrency ? `${formatCurrency(expense.amount, expense.currency)} (≈ ${formatCurrency(expense.amount * (expense.exchangeRate || 1), baseCurrency)})` : formatCurrency(expense.amount, baseCurrency)}</td><td className="py-3 text-right"><Button size="icon" variant="ghost" onClick={() => setDialog(expense)}><Pencil className="h-4 w-4" /></Button><Button size="icon" variant="ghost" onClick={() => void remove(expense)}><Trash2 className="h-4 w-4 text-destructive" /></Button></td></tr>)}</tbody>
            </table>
            {!filtered.length && <div className="py-10 text-center text-sm text-muted-foreground">No expenses match these filters.</div>}
          </div>
        </CardContent>
      </Card>
      {dialog && <ExpenseDialog expense={dialog} expenses={expenses} baseCurrency={baseCurrency} onClose={() => setDialog(null)} onSave={(expense) => { setExpenses((current) => dialog === "new" ? [expense, ...current] : current.map((item) => item.id === expense.id ? expense : item)); setDialog(null); }} />}
    </div>
  );
}

function ExpenseDialog({ expense, expenses, baseCurrency, onClose, onSave }: { expense: ExpenseRecord | "new"; expenses: ExpenseRecord[]; baseCurrency: string; onClose: () => void; onSave: (expense: ExpenseRecord) => void }) {
  const editing = expense !== "new";
  const { register, handleSubmit, watch, formState: { errors, isSubmitting } } = useForm<ExpenseFormInput, unknown, ExpenseForm>({ resolver: zodResolver(expenseSchema), defaultValues: editing ? { ...expense, date: expense.date.slice(0, 10), notes: expense.notes ?? "", currency: expense.currency || baseCurrency } : { date: format(new Date(), "yyyy-MM-dd"), merchant: "", amount: 0, category: "Other", notes: "", currency: baseCurrency } });
