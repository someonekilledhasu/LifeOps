...
export async function createExpense(
  ...
  if (userId === GUEST_USER_ID) {
    return { id: `guest-expense-${Date.now()}`, date: new Date(input.date).toISOString(), merchant: input.merchant, amount: input.amount, category: input.category, notes: input.notes, source: input.source ?? "manual" };
  }
  ...
  return { id: expense.id, date: expense.date.toISOString(), merchant: expense.merchant, amount: Number(expense.amount), category: categoryFromDb[expense.category], notes: expense.notes ?? undefined, source: expense.source };
}
...
export async function updateExpense(
  ...
  if (userId === GUEST_USER_ID) {
    return { id: expenseId, date: new Date(input.date).toISOString(), merchant: input.merchant, amount: input.amount, category: input.category, notes: input.notes, source: "manual" };
  }
...
export async function createFoodDecision(
  ...
  if (userId === GUEST_USER_ID) {
    return { id: `guest-food-${Date.now()}`, createdAt: new Date().toISOString(), mood: input.mood, mode: input.mode, suggestions: input.suggestions };
  }
  const decision = await prisma.foodDecision.create({
    data: {
      ...
      mode: input.mode,
      health: input.health,
      suggestions: input.suggestions as unknown as Record<string, unknown>[],
    },
  });
  return { id: decision.id, createdAt: decision.createdAt.toISOString(), mood: decision.mood, mode: decision.mode, suggestions: decision.suggestions as FoodDecisionRecord["suggestions"] };
}
...
export async function createMessage(
  ...
  if (userId === GUEST_USER_ID) {
    return { id: `guest-message-${Date.now()}`, ...input, subject: input.subject ?? "", createdAt: new Date().toISOString() };
  }
...
const defaultSettings: UserSettingsRecord = { name: "User", monthlyBudget: 30000, currency: "INR", dietaryPreference: "flexible", favoriteCuisines: ["Indian", "Asian", "Mediterranean"], darkMode: false };

export async function getSettings(userId: string): Promise<UserSettingsRecord> {
  if (userId === GUEST_USER_ID) return { ...defaultSettings, name: "Hasini" };
...
export async function upsertSettings(
  ...
  if (userId === GUEST_USER_ID) return input;
